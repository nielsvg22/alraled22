        ...
        <div className="max-w-6xl mx-auto px-4 md:px-10 h-12 flex items-center justify-between gap-4">
          ...
          <ul className="hidden md:flex items-center gap-1">
            ...
            <li className="relative"
              ...
              onMouseLeave={() => { closeTimeoutRef.current = setTimeout(() => { setShowCategories(false); setHoveredCategory(null); }, 120); }}>
              <Link to="/producten"
              ...
              {showCategories && categories.length > 0 && (
                <div
                  className="absolute top-full left-1/2 -translate-x-1/2 mt-1 bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden z-50 flex"
                  style={{ width: '760px' }}
                  ...
                  <div className="w-[280px] border-r border-gray-100 py-2">
                    <Link to="/producten" onClick={() => setShowCategories(false)}
                      className="block px-5 py-2.5 text-sm font-bold text-gray-900 hover:bg-gray-50 transition-colors border-b border-gray-100 mb-1">
                      Alle producten &rarr;
                    </Link>
                    {categories.map(cat => (
                      <Link
                        ...
                        className="flex items-center gap-3 px-5 py-2.5 text-sm font-medium transition-colors border-b border-gray-50 last:border-0 hover:bg-primary/5"
                        style={hoveredCategory?.id === cat.id ? { backgroundColor: 'rgba(59,130,246,0.05)', color: '#2563eb' } : { color: '#374151' }}>
                  ...
                  {/* Right: image preview */}
                  <div className="w-[480px] relative bg-gray-50 flex items-center justify-center p-6">
                    {hoveredCategory ? (
                      <div className="w-full h-full flex flex-col items-center justify-center gap-4 animate-in fade-in duration-200">
                        ...
                        <div className="text-center">
                          <p className="text-sm font-bold text-gray-900">{hoveredCategory.name}</p>
                          {hoveredCategory.description && (
                            <p className="text-xs text-gray-500 mt-1 max-w-xs">{hoveredCategory.description}</p>
                          )}
                          {hoveredCategory.productCount > 0 && (
                            <p className="text-xs text-primary font-semibold mt-1">{hoveredCategory.productCount} producten</p>
                          )}
                        </div>
                      </div>
                    ) : (
                      <div className="text-center text-gray-300">
                        ...
                        <p className="text-xs font-medium">Hover over een categorie</p>
                      </div>
                    )}
                  </div>
                </div>
