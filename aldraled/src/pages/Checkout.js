          ...
          <div className="lg:sticky lg:top-32 h-fit bg-gray-50 rounded-[3rem] p-10 md:p-12 space-y-10">
            ...
            <div className="space-y-6 max-h-[400px] overflow-y-auto pr-4">
              {cartItems.map((item) => (
                <div key={item.id} className="flex justify-between items-center gap-6">
                  ...
                  <p className="font-black text-secondary italic text-lg">{formatPrice(item.price * item.quantity * vatMultiplier)}</p>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-4 pt-8 border-t-2 border-dashed border-gray-200">
              <div className="flex justify-between text-gray-400 font-bold uppercase tracking-widest text-xs">
                <span>Subtotaal</span><span>{formatPrice(cartTotal * vatMultiplier)}</span>
              </div>
              {appliedDiscount && (
                <div className="flex justify-between text-emerald-600 font-bold uppercase tracking-widest text-xs italic">
                  <span>Korting</span><span>-{formatPrice(appliedDiscount.discountAmount * vatMultiplier)}</span>
                </div>
              )}
              <div className="flex justify-between text-gray-400 font-bold uppercase tracking-widest text-xs">
              ...
              <div className="flex justify-between items-end pt-4">
                ...
                <div className="text-right">
                  <p className="text-5xl font-black text-secondary italic tracking-tighter">{formatPrice(finalTotal * vatMultiplier)}</p>
                  <button
                    type="button"
