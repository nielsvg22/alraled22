...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const msg = err.response?.data?.error || err.response?.data?.message || err.message || t('register.fallbackError');
      setError(msg);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-primary/5 rounded-full -mr-72 -mt-72 blur-[100px]"></div>
      
      <div className="sm:mx-auto sm:w-full sm:max-w-md relative z-10 text-center">
        <h2 className="text-6xl font-black text-secondary uppercase italic tracking-tighter leading-none mb-4">
          {t('register.titlePrefix')}<span className="text-primary">{t('register.titleHighlight')}</span>
        </h2>
        <p className="text-gray-400 font-bold uppercase tracking-widest text-xs">{t('register.subtitle')}</p>
      </div>
        ...
        <div className="bg-white py-12 px-10 shadow-2xl rounded-[3rem] border border-gray-100">
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="space-y-2">
              <label className="text-xs font-black text-secondary uppercase tracking-widest px-2">{t('register.name')}</label>
              <input
                type="text"
                required
            ...
            <div className="space-y-2">
              <label className="text-xs font-black text-secondary uppercase tracking-widest px-2">{t('register.email')}</label>
              <input
                type="email"
                required
                className="w-full bg-gray-50 border-2 border-gray-100 rounded-2xl px-6 py-4 focus:border-primary outline-none transition-all font-medium"
            ...
            <div className="space-y-2">
              <label className="text-xs font-black text-secondary uppercase tracking-widest px-2">{t('register.password')}</label>
              <input
                type="password"
                required
                className="w-full bg-gray-50 border-2 border-gray-100 rounded-2xl px-6 py-4 focus:border-primary outline-none transition-all font-medium"
